...
Uses the exact LangChain + HuggingFace + Chroma pipeline provided in the problem statement.
Called internally by the Spring Boot application.
"""
import os
import logging
...
# ---- Documents (as-is from the problem statement) ----
doc1 = Document(
    page_content=(
...
IDS = ["csk", "mi", "rcb", "kkr", "rr", "srh", "dc", "pbks", "gt", "lsg"]
...
def build_vector_store():
    ...
    if not existing.get("ids"):
        logger.info("Seeding %d IPL documents into the vector store...", len(DOCS))
        store.add_documents(documents=DOCS, ids=IDS)
    else:
        logger.info("Vector store already contains %d docs, skipping seed.",
                    len(existing["ids"]))
    vector_store = store
...
app = FastAPI(title="IPL RAG Service", lifespan=lifespan)
...
@app.get("/health")
def health():
    return {"status": "ok", "ready": vector_store is not None}
...
def rag_query(req: QueryRequest):
    ...
    return QueryResponse(
        query=req.query,
        results=[
            RetrievedDoc(team=d.metadata.get("team", "Unknown"), content=d.page_content)
            for d in results
        ],
    )
